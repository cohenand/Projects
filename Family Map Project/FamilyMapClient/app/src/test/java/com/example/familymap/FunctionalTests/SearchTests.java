...
public class SearchTests {
    ...
    @Test
    public void searchAllTestForNothing() {
        Event[] output;
        settings = new Settings(false,false,false,true,true,true,true);
        assertEquals(0,familyTree.searchEventResults("dddd",settings).size());

    }

    @Test
    public void searchOneForSomethingAndNothing() {
        Event[] output;
        settings = new Settings(false,false,false,false,false,true,true);
        assertEquals(0,familyTree.searchEventResults("dddd",settings).size());
        assertNotEquals(0,familyTree.searchEventResults("shei",settings).size());
        assertEquals(familyTree.searchEventResults("sheila",settings).size(),familyTree.searchEventResults("shei",settings).size());
